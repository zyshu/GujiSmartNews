 ...
 */
public class HotNewsBestMultipleRecycleAdapter extends BaseMultiItemQuickAdapter<HotNewsBestModel.Articles, BaseViewHolder> implements OnBannerListener {
     ...
     */
    public HotNewsBestMultipleRecycleAdapter(Context context, List<HotNewsBestModel.Articles> data) {
        super(data);
        this.context = context;
        addItemType(HotNewsBestModel.Articles.TYPE_HORIZAONTAL_IMAGE, R.layout.item_index_horizontal_list);
        addItemType(HotNewsBestModel.Articles.TYPE_EMPTY_IMAGE, R.layout.item_index_zeroempty_image);
        addItemType(HotNewsBestModel.Articles.TYPE_TYADMODE_IMAGE, R.layout.item_index_tyad_menu);
        addItemType(HotNewsBestModel.Articles.TYPE_TOP_BANNER, R.layout.item_index_banner_with_menu);
        addItemType(HotNewsBestModel.Articles.TYPE_SINGLE_IMAGE, R.layout.item_index_single_image);
        addItemType(HotNewsBestModel.Articles.TYPE_THREE_IMAGE, R.layout.item_index_three_image);
        addItemType(HotNewsBestModel.Articles.TYPE_SINGLE_BIG_IMAGE, R.layout.item_index_single_big_image);
    }

    @Override
    protected void convert(BaseViewHolder helper, HotNewsBestModel.Articles articles) {
    ...
    private void bindTopBannerData(BaseViewHolder helper, final HotNewsBestModel.Articles item, int position) {
...
//        mBanner.setBannerStyle()
        ...
        initAnim();
        //数据
        for (HotNewsBestModel.Articles.Pics a : item.getPics()) {
     ...
     * @param position
     */
    private void bindSingleNewsData(BaseViewHolder helper, HotNewsBestModel.Articles item, int position) {
...
//                ((TextView)helper.getView(R.id.tvZTText)).setText(item.flag.text == null ? "" : item.flag.text);
     ...
     */
    private void bindSingleBigNewsData(BaseViewHolder helper, HotNewsBestModel.Articles item, int adapterPosition) {
     ...
     * @param adapterPosition
     */
    private void bindTYAdImageData(BaseViewHolder helper, HotNewsBestModel.Articles item, int adapterPosition) {
     ...
     * @param position
     */
    private void bindMultiImgNewsData(BaseViewHolder helper, HotNewsBestModel.Articles item, int position) {
        if (item.getPics().size() >= 3) {
...
//                ((TextView)helper.getView(R.id.tvZTText)).setText(item.flag.text == null ? "" : item.flag.text);
     ...
     * @param position
     */
    private void bindEmptyImgNewsData(BaseViewHolder helper, HotNewsBestModel.Articles item, int position) {
        helper.setText(R.id.Tarticle_title, item.title == null ? "" : item.title);
        helper.setText(R.id.tvTSourceName, item.mediaName == null ? "" : item.mediaName);
        helper.setText(R.id.tvTcommetnum, item.commentNum + "");
        helper.setText(R.id.tvDate, item.virtualTime + "");
     ...
     * @param adapterPosition
     */
    private void bindHorizaontalImgNewsData(BaseViewHolder helper, HotNewsBestModel.Articles item, int adapterPosition) {
...
//        List<HomeTophotIndex.Articles.HotSpecialNews> hotSpecialNews = item.getHotSpecialNews();
